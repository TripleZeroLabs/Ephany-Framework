"""
Tests for the asset summary endpoint.
...
def summary_url(asset):
    return f"/api/assets/{asset.id}/summary/"


@override_settings(**OPEN_API)
class AssetSummaryTests(TestCase):
    @classmethod
    ...
    def get_summary(self, asset):
        response = self.client.get(summary_url(asset))
    ...
    def test_counts_only_the_latest_snapshot_per_project(self):
        ...
        """
        payload = self.get_summary(self.display)

        self.assertEqual(AssetInstance.objects.filter(asset=self.display).count(), 24)
        self.assertEqual(payload["totals"]["total_installed"], 8)
        self.assertEqual(payload["totals"]["site_count"], 2)
    ...
    def test_a_newer_snapshot_supersedes_the_previous_count(self):
        ...
        self.assertEqual(payload["totals"]["total_installed"], 3)
    ...
    def test_basis_is_stated_in_the_response(self):
        ...
        self.assertEqual(
            self.get_summary(self.display)["totals"]["basis"],
    ...
    def test_site_rows_identify_the_project_and_snapshot(self):
        site = self.get_summary(self.display)["sites"][0]
    ...
    def test_replacement_uses_the_cheapest_quote_and_lists_them_all(self):
        replacement = self.get_summary(self.display)["replacement"]
    ...
    def test_money_is_a_string_not_a_float(self):
        """Currency through a float is a bug waiting on a big enough portfolio."""
        replacement = self.get_summary(self.display)["replacement"]
    ...
    def test_asset_installed_nowhere_returns_an_empty_summary(self):
        payload = self.get_summary(self.orphan)

        self.assertEqual(payload["totals"]["total_installed"], 0)
        self.assertEqual(payload["totals"]["site_count"], 0)
        self.assertEqual(payload["sites"], [])

    def test_replacement_is_null_when_no_vendor_quotes_the_asset(self):
        self.assertIsNone(self.get_summary(self.orphan)["replacement"])

    def test_unknown_asset_is_404(self):
        self.assertEqual(self.client.get("/api/assets/999999/summary/").status_code, 404)
    ...
    def test_summary_runs_in_a_constant_number_of_queries(self):
        ...
        with self.assertNumQueries(3):
            ...
            self.client.get(summary_url(self.display))


@override_settings(**OPEN_API)
class AssetSummaryAuthTests(TestCase):
    ...
    def test_requires_a_credential_when_anonymous_access_is_closed(self):
        self.assertEqual(self.client.get(summary_url(self.asset)).status_code, 401)
